...
class KDA_RNN(SequentialModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--rnn_model', type=str, default='lstm',
                            help='Method of rnn used: lstm, gru, transformer')
    ...
    def __init__(self, args, corpus):
        ...
        self.rnn_model = args.rnn_model.lower()
    ...
    def _define_params(self):
        ...
        elif self.rnn_model == 'transformer':
    ...
    def rec_forward(self, feed_dict):
        ...
        if self.rnn_model == 'lstm':
            rnn_output, (u_vectors, _) = self.rnn(user_seq_emb_dropout)
            u_vectors = u_vectors[-1]
            u_vectors = self.dense(u_vectors)
        elif self.rnn_model == 'gru':
            rnn_output, u_vectors = self.rnn(user_seq_emb_dropout)
            u_vectors = u_vectors[-1]
            u_vectors = self.dense(u_vectors)
        elif self.rnn_model == 'transformer':
            u_vectors = self.rnn(user_seq_emb_dropout)
            u_vectors = u_vectors[-1]
            print(f'u_vectors.shape: {u_vectors.shape}')
            print(f'u_vectors.shape after T: {u_vectors.transpose(0,1).shape}')        
        ...
        if self.pooling == 'attention':
            ...
            his_vector = context.sum(dim=-2)  # B * -1 * V
        else:
